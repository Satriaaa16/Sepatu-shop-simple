...
<head>
    ...
    <style>
        ...
        /* Add a separator line between buttons */
        .separator {
            height: 1px;
        ...
        /* Add the icon styling for love and cart */
        .product-card .action-icons i {
            font-size: 20px;
            cursor: pointer;
            transition: color 0.3s ease;
        }

        .product-card .action-icons i.loved {
            color: red;
        }

        .product-card .action-icons i.carted {
            color: #FF5722;
        }
    </style>
</head>
<body>
        ...
        <?php if (count($products) > 0): ?>
            <?php foreach ($products as $product) { ?>
                <div class="product-card">
                    ...
                    <!-- Action Icons (Love and Cart) -->
                    <div class="action-icons">
                        <i class="fas fa-heart" onclick="toggleLike(this)"></i>
                        <i class="fas fa-shopping-cart" onclick="toggleCart(this)"></i>
                    </div>
                </div>
            <?php } ?>
        <?php else: ?>
    ...
    <script>
        ...
        // Toggle the like icon color on click
        function toggleLike(icon) {
            icon.classList.toggle('loved');
        }

        // Toggle the cart icon color on click
        function toggleCart(icon) {
            icon.classList.toggle('carted');
        }
    </script>
